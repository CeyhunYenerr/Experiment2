...
$(function() {
  ...
  function set_settings() {
    ...
    settings.defaultredirect = 'https://yasarpsych.qualtrics.com/jfe/form/SV_8wE7rz3W9nQ7FCC';
    ...
    settings.condition_1_likes = [10000, 11000,15000,35000,80000,100000,110000,150000,20000]; 
    ...
    settings.condition_3_likes = [10000, 13000,19000,35000,80000,100000,110000,150000]; 
	...
	settings.condition_1_adjusted_likes = [12000, 15000,19000,35000,80000,100000,110000,150000,20000]; // 9
    ...
    settings.likes_by = ['Tahsinnn','Kenaan','EsraT','Aynurella','Dnz','Kerem','Emre','Lidya','Zeynep','Arya','Melisa']; 
  }
  ...
  function init_name() {
  	...
  	$('#submit_username').on('click',function() {
  		...
  		if(uname == "") {
  			error = 1;
  			errormsg = 'Lütfen Metin Giriniz';
  			uname = "undefined";
  		}
  		if(not_alphanumeric(uname)) {
  			error = 1;
  			errormsg = 'Sadece harfleri kullanınız (boşluk olmadan)';
  		}  		
    	...
    	$('#submit_avatar').on('click',function() {
    		if($('.selected').length == 1) {
  			...
  			window.avatarexport = /avatar_([^\s]+)/.exec(window.avatar)[1];
    			...
    			alertify.log("Lütfen bir avatar seçiniz","Hata");
    		}
    	});
  ...
  function init_text() {
  	...
  	$('#submit_text').on('click',function() {
  		...
  		if($('#description').val() !== "" && $('#description').val().length < 140) {
  			...
  			error = 1;
  			errormsg = 'Lütfen biraz daha yazınız';
			}
  		if($('#description').val().length > 401) {
  		
  			error = 1;
  			errormsg = ' Lütfen kelime sayısını azaltın';
  		}  		
  		if(error == 0) {
  			$('#text').hide();
    ...
    // When user receives likes
	  $('.userslikes').each(function() {
  		...
  		{ 
  			...
  			themsg = usernames[i] + "Tanıtımınızı Beğendi";
  ...
  $(window).bind('beforeunload', function(){
    return 'Çalışmadan çıkmak istediğinize emin misiniz?';
  });   
